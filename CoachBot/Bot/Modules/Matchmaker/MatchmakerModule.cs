...
namespace CoachBot.Modules.Matchmaker
{
    public class MatchmakerModule : ModuleBase
    {
        ...
        public MatchmakerModule(MatchmakerService service, ConfigService configService, StatisticsService statisticsService, ServerService serverService)
        {
            _service = service;
        ...
        {
            var regionId = _configService.Config.Channels.First(c => c.Id == Context.Message.Channel.Id).RegionId;
            if (regionId == 0)
            {
            ...
            {
                await ReplyAsync("", embed: _serverService.GenerateServerListEmbed(regionId));
            }
        }
